...
class LoadDataDespesasSaude:
    def __init__(self, conn) -> None:
        self.__path = os.path.dirname(os.path.realpath(__file__))
        self.__conn = conn

    def load(self, transform_html_data, year):
        logging.info('load despesas saude')

        seq_orgao = transform_html_data[0]['seq_orgao']

        self.delete_despesas_saude(year, seq_orgao)

        for row in transform_html_data:
            logging.info('Importa nova despesa saude')

            self.create_despesa_saude(row)

    def delete_despesas_saude(self, year, seq_orgao):
        logging.info('Deleta despesas saude por ano')

        cursor = self.__conn.cursor()

        query = "DELETE FROM despesas_saude WHERE num_ano_referencia = %s AND seq_orgao = %s"

        where = (year, seq_orgao)

        cursor.execute(query, where)

    def create_despesa_saude(self, row):
        cursor = self.__conn.cursor()
        ...
        query = "INSERT INTO despesas_pessoas (cod_municipio, seq_orgao, seq_unidade, num_ano_referencia, " \
        ...
        values = (
            cod_municipio, seq_orgao, seq_unidade, num_ano_referencia, num_mes_referencia, dsc_funcao, dsc_subfuncao,
            dsc_programa, dsc_acao, dsc_subacao, dsc_nat_despesa, dsc_fonte_recurso, num_empenho, dat_empenho,
            vlr_empenho,
            vlr_ref_empenho, vlr_anu_empenho, vlr_liquidado, vlr_anu_liquidado, vlr_pagamento, vlr_anu_pagamento,
            vlr_outras_baixas, vlr_anu_outras_baixas, vlr_rsp_proc, vlr_rsp_nao_proc, num_versaoarq, created_at,
            updated_at)
